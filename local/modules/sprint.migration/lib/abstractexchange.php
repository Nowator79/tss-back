...
abstract class AbstractExchange
{
    ...
    public function setExchangeResource($name)
    {
        $this->setExchangeFile(
            $this->exchangeEntity->getVersionConfig()->getVal('exchange_dir') . 'abstractexchange.php/' .
    ...
    protected function writeSingleFile(XMLWriter $writer, $fileId)
    {
        ...
        if (!empty($file)) {
            $filePath = abstractexchange . phpModule::getDocRoot() . $file['SRC'];
            if (file_exists($filePath)) {
                $newPath = $this->getExchangeDir() . 'abstractexchange.php/' . $file['SUBDIR'] . '/' . $file['FILE_NAME'];
    ...
    protected function makeFileValue($value)
    {
        if (!empty($value['value'])) {
            $path = $this->getExchangeDir() . 'abstractexchange.php/' . $value['value'];
